...
void Seek_PublicationTime()
{
	...
	do 
	{
		...
		if (flag)
		{
			...
			{
				sum += (*p++ - '0') * (int)pow((double)10, string_len-i-1);
			}
...
void Seek_BookPrice()
{
	...
	do 
	{
		...
		if (flag)
		{
			...
			{
				price1 += ((double)price_str[index - 1 - i] - '0') * pow((double)10, i);
			}
			int exponent = -1;
			for (int j = index + 1; j < temp; j++)
			{
				price1 += ((double)price_str[j] - '0') * pow((double)10, exponent--);
			}
			price = price1;
		}
...
int Seek_BookID()
{
	...
	do 
	{
		...
		if (flag)
		{
			p = str;
			for (int i = 0; i < string_length; i++)
				sum = sum + (*p++ - '0') * (int)pow((double)10, string_length - i - 1);
			bookid = sum;
		}
		else 
		...
		do 
		{
			...
			if (_flag1)
			{
				...
				for (int i = 0; i < string_len1; i++)
					sum1 += (*p1++ - '0') * (int)pow((double)10, string_len1 - i - 1);
				from = sum1;
			}
		...
		do 
		{
			...
			if (_flag2)
			{
				...
				for (int i = 0; i < string_len2; i++)
					sum2 += (*p2++ - '0') * (int)pow((double)10, string_len2 - i - 1);
				to = sum2;
			}
			else
				cout << endl << "输入不符合规范，请重新输入!" << endl << endl;
